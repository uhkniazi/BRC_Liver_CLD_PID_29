...
dfLimmma.fat = topTable(fit, coef = 2, adjust='BH', number=Inf)
table(dfLimmma.fat$adj.P.Val < 0.01)

dfLimmma.muscle = topTable(fit, coef = 3, adjust='BH', number=Inf)
table(dfLimmma.muscle$adj.P.Val < 0.01)
...
## add annotations
identical(rownames(dfLimmma.fat), rownames(dfLimmma.muscle))

df = dfAnnotation[dfAnnotation$ID %in% rownames(dfLimmma.fat), ]
dim(df)
i = match(rownames(dfLimmma.fat), df$ID)
df = df[i,]
identical(rownames(dfLimmma.fat), as.character(df$ID))
dfLimmma.fat$REFSEQ=as.character(df$GB_ACC)
## get other related information
df = select(x, keys = dfLimmma.fat$REFSEQ, columns = c('ENTREZID', 'SYMBOL'),
            keytype = 'REFSEQ')
identical(as.character(dfLimmma.fat$REFSEQ), as.character(df$REFSEQ))
head(df)
dfLimmma.fat = cbind(dfLimmma.fat, df[,c(2,3)]) 
head(dfLimmma.fat)

## second tissue
identical(rownames(dfLimmma.muscle), rownames(dfLimmma.muscle))

df = dfAnnotation[dfAnnotation$ID %in% rownames(dfLimmma.muscle), ]
dim(df)
i = match(rownames(dfLimmma.muscle), df$ID)
df = df[i,]
identical(rownames(dfLimmma.muscle), as.character(df$ID))
dfLimmma.muscle$REFSEQ=as.character(df$GB_ACC)
## get other related information
df = select(x, keys = dfLimmma.muscle$REFSEQ, columns = c('ENTREZID', 'SYMBOL'),
            keytype = 'REFSEQ')
identical(as.character(dfLimmma.muscle$REFSEQ), as.character(df$REFSEQ))
head(df)
dfLimmma.muscle = cbind(dfLimmma.muscle, df[,c(2,3)]) 
head(dfLimmma.muscle)

# put both tables in same order
table(rownames(dfLimmma.fat) %in% rownames(dfLimmma.muscle))
dfLimmma.fat = dfLimmma.fat[rownames(dfLimmma.muscle),]
identical(rownames(dfLimmma.fat), rownames(dfLimmma.muscle))

## produce the plots 
table(dfLimmma.fat$adj.P.Val < 0.01)
range(dfLimmma.fat$logFC)
f_plotVolcano(dfLimmma.fat, 'Fat vs Liver', p.adj.cut = 0.01, fc.lim = range(dfLimmma.fat$logFC) )#, fc.lim=c(-2.5, 2.5))
f_plotVolcano(dfLimmma.muscle, 'Muscle vs Liver', p.adj.cut = 0.01, fc.lim = range(dfLimmma.muscle$logFC) )#, fc.lim=c(-2.5, 2.5))

plotMeanFC(dfLimmma.fat$AveExpr, dfLimmma.fat, 0.01, 'Fat vs Liver')
plotMeanFC(dfLimmma.muscle$AveExpr, dfLimmma.fat, 0.01, 'Muscle vs Liver')

## save the results 
write.csv(dfLimmma.fat, file='results/DEAnalysis_FatVSLiver.xls')
write.csv(dfLimmma.muscle, file='results/DEAnalysis_MuscleVSLiver.xls')
