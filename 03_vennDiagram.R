...
venn.diagram(lVenn, filename = 'results/figures/venn_CLDBA_contrasts_vsLiver.tif', margin=0.1)
...
names(lVenn) = cvTitle
venn.diagram(lVenn, filename = 'results/venn_up_down_CLDBA_contrasts_vsLiver.tif', margin=0.1)
...
dfCommonGenes = data.frame(mCommonGenes, sig.pvals=rowSums(mCommonGenes), 
                           groups=cp, Symbol=ldfData[[1]][cvCommonGenes, 'SYMBOL'],
                           ENTREZID=ldfData[[1]][cvCommonGenes, 'ENTREZID'])
...
df = AnnotationDbi::select(org.Hs.eg.db, keys = as.character(dfCommonGenes$ENTREZID), columns = c('SYMBOL', 'GENENAME'), keytype = 'ENTREZID')
identical(as.character(dfCommonGenes$ENTREZID), df$ENTREZID)
